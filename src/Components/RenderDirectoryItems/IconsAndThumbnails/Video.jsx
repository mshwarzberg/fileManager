...
function Video({ directoryItem, setControllers }) {
  const { name, thumbPath, duration, path, size } = directoryItem;
  ...
  useEffect(() => {
    ...
    // getMediaMetadata(state.currentDirectory, name).then((result) => {
    //   console.log(result);
    //   sessionStorage.setItem(path, JSON.stringify(result));
    //   setDirectoryItems((prevItems) => {
    //     return prevItems.map((item) => {
    //       if (item.name === name) {
    //         return {
      ...
      <div className="block-container">
        ...
        <img
          ...
          onError={(e) => {
            ...
            setTimeout(() => {
              if (
                srcAttempts >= directoryItems.indexOf(directoryItem) + 2 ||
                size < 300000
              ) {
                setDirectoryItems((prevItems) => {
                  return prevItems.map((item) => {
